...
for ($j = 0; $j < $mylength; $j++) {
	...
	elseif (($rows[$j]['inventorySize'] == '15 cans') && ($rows[$j]['servingSize'] == '12 ounces')) {  // weirdo case of 15 cans - Founders All Day IPA
          ...
          // a fix to make the mystery divide by zero error go away - we have a bad entry to the database somewhere...
          $num_servings = 1;
        }
...
for ($j = 0; $j < $mylength; $j++) {
	...
	echo "<a href=\"".$rows2[$j]['BA_link']."\" rel=\"external\">".$rows2[$j]['brand']." ".$rows2[$j]['type']."</a> (".number_format((float)($rows2[$j]['ABV']), 1, '.', '')."%)<br />";
} 
